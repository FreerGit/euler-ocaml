...
let%test_unit "sum of squares" =
  [%test_result: int] (sum_of_squares first_ten) ~expect:385
;;

let%test_unit "square of sums" =
  [%test_result: int] (square_of_sums first_ten) ~expect:3025
;;

let%test_unit "difference (ten)" = [%test_result: int] (difference first_ten) ~expect:2640

let%test_unit "difference (hundred)" =
  [%test_result: int] (difference first_hundred) ~expect:25164150
;;
